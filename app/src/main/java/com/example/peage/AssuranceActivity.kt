...
class AssuranceActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var buttonUpload = findViewById<Button>(R.id.buttonUpload2)
        button.setOnClickListener {
            val gallery = Intent(Intent.ACTION_PICK, MediaStore.Images.Media.INTERNAL_CONTENT_URI)
            startActivityForResult(gallery, pickImage)
        }
        buttonNext = findViewById(R.id.goToCard)
    ...
    private fun uploadToFirebase(uri : Uri){
        ...
        var fileRef = StorageRef.child("assurance_images/" + System.currentTimeMillis().toString() + "." + getFileExtension(uri))
